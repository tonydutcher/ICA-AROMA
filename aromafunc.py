...
def antsregister2MNI(fslDir, inFile, outFile, func2anat, affmat, warp, nn=False):
...
def mask_sum(fslDir, image, mask_file=None):
...
def feature_spatial(fslDir, tempDir, aromaDir, melIC, gray=None):
    ...
    for i in range(0,numICs):
        ...
        csfSum = mask_sum(fslDir, tempIC,
                          os.path.join(aromaDir, 'mask_csf.nii.gz'))
        edgeSum = mask_sum(fslDir, tempIC,
                           os.path.join(aromaDir, 'mask_edge.nii.gz'))
        outSum = mask_sum(fslDir, tempIC,
                          os.path.join(aromaDir, 'mask_out.nii.gz'))
        if gray is not None:
            graySum = mask_sum(fslDir, tempIC, gray)
        else:
            graySum = 0

        # Determine edge and CSF fraction
        if totSum > 0:
            edgeFract[i] = (outSum + edgeSum - graySum)/(totSum - csfSum)
